...
#### 精选主页（首页）
- **Hero 背景大图**：管理员可在后台上传任意图片作为首页 Banner 背景，鼠标移动触发平滑视差效果，滚动时图片自然位移；未设置时回退到主题色。
...
- **群组聊天**：创建群组，邀请多名关注的用户
- **撤回消息**：发送后 2 分钟内可撤回
- 导航栏实时显示未读消息数（AJAX 3秒轮询）
- 双向黑名单过滤：被拉黑的用户无法发起私信
...
#### 主页管理（homepage.php）
- **Hero 背景图管理**：上传 JPG/PNG/WebP（≤5MB，建议 1920×600+），实时预览叠加渐变遮罩效果，支持删除还原纯色背景
- 可视化 6 槽位布局预览
...
│   ├── sections.php            # 板块管理
│   ├── homepage.php            # 主页槽位 + Hero 背景图管理
│   ├── clubs.php               # 社团管理（审核/改名/停用）
│   ├── messages.php            # 私信内容查阅（合规审核）
│   ├── settings.php            # AI 配置与调用统计
│   └── logs.php                # 管理员操作记录
│
...
│   ├── ai_test.php             # DeepSeek API 连通性测试
│   ├── message_action.php      # 私信发送/撤回/群组操作
│   └── hero_bg_upload.php      # 首页 Hero 背景图上传/删除
│
├── api/                        # AJAX 数据接口（返回 JSON）
│   ├── search_posts.php        # 搜索帖子（主页管理用）
│   ├── user_search.php         # 搜索用户（私信选人用）
│   ├── messages_poll.php       # 轮询未读消息数
│   ├── get_following.php       # 获取关注列表（私信转发用）
│   └── upload_image.php        # 帖子内图片上传
│
├── includes/                   # 公共组件
...
1. 以管理员登录，进入**管理后台 → 主页管理**
2. 在"首页背景大图"区域点击"选择文件"，上传 JPG/PNG/WebP（≤5MB，建议 1920×600）
3. 点击"上传"后刷新首页即可看到效果
4. 背景图路径存储在 `site_settings` 表（`key='hero_bg'`），文件保存在 `uploads/hero/` 目录
